...
if (strlen($_SESSION['mhslogin']) == 0) {
    ...
    // if (isset($_POST['file_array']) && isset($_FILES['file_array'])) {
    ...
    //         if (move_uploaded_file($tmp_name_array[$i], "file/" . $name_array[$i])) {
    ...
    //     }
    // }
    $limit = 1 * 1024 * 1024; //2MB. Bisa diubah2
    ...
    if (isset($_FILES['syarat'])) {
        ...
        $jumlahFile = count($_FILES['syarat']['name']);
        for ($i = 0; $i < $jumlahFile; $i++) {
            $cnt = 0;

            $namasyarat = $_POST["nama_syarat"];
            $namafile = $_FILES['syarat']['name'][$i];
            $tmp = $_FILES['syarat']['tmp_name'][$i];
            $type = $_FILES['syarat']['type'][$i];
            $error = $_FILES['syarat']['error'][$i];
            $size = $_FILES['syarat']['size'][$i];
        }
        ...
        // INSERT INTO syarat_daftar (kd_daftar, kd_syarat_bsw, isi_syarat) values ('72160057','6','72160057_Scan Bukti Pembayaran Listrik Bulan Terakhir_5dab1961e93a7.pdf')
        //rename the file
        // $namasyarat2 = $_POST["nama_syarat"];
        // for ($i = 0; $i < count($namasyarat2); $i++) {
        //     $filename2   = $nim . "_" . $namasyarat2[$i] . "_" . uniqid() . "_" . time();
        //     $extension  = pathinfo($namafile, PATHINFO_EXTENSION); // .pdf
        //     $basename2   = $filename2 . "." . $extension;
        //     move_uploaded_file($tmp, "file/" . $basename2[$i]);

        // }
        // $filename   = $nim . "_" . $namasyarat[$i++] . "_" . uniqid() . "_" . time(); // 5dab1961e93a7-1571494241
        // $extension  = pathinfo($namafile, PATHINFO_EXTENSION); // .pdf
        // $basename   = $filename . "." . $extension; // 5dab1961e93a7_1571494241.pdf
        // Code for move image into directory
        // move_uploaded_file($tmp, "file/" . $basename);
        ...
        // for ($i = 0; $i < count($tmp_name_array); $i++) {
        //     $filename   = $nim . "_" . $namasyarat2[$i] . "_" . uniqid() . "_" . time();
        //     $extension  = pathinfo($name_array[$i], PATHINFO_EXTENSION); // .pdf
        //     $basename2   = $filename . "." . $extension;
        //     if (move_uploaded_file($tmp_name_array[$i], "file/" . $filename)) {
        //         echo $name_array[$i] . " upload is complete<br>";
        //     } else {
        //         echo "move_uploaded_file function failed for " . $name_array[$i] . "<br>";
        //     }
        ...
        $namasyarat2 = $_POST["nama_syarat"];
        $cnt = 0;
        $totalFileSize = array_sum($_FILES['syarat']['size']);
        echo 'upload size : ' . $totalFileSize . ' bytes';
        $maxFileSize = 1 * 1024 * 1024;
        if ($totalFileSize > $maxFileSize) {
            echo 'Your files exceed the limit of 1MB capacity';
            $_SESSION['error'] = "'" . $totalFileSize . "'.Ukuran gambar yang diupload melebihisss yang diizinkan";
            header('location: form.php?id=1');
            exit();
        } else {
                ...
                <div class="card-body">
                    ...
                    ?>
                        ...
                        ?>
                                        ...
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group input-group-merge">
                                                    ...
                                                    <input type="text" name="pekerjaan-ortu" class="form-control" id="pekerjaan-ortu" placeholder="Pekerjaan Orang Tua" title="Pekerjaan Orang Tua Anda" oninvalid="this.setCustomValidity('Selahkan Masukkan Pekerjaan Orangtua Anda.')" oninput="setCustomValidity('')" required>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="custom-file">
                                                        ...
                                                        <input type="text" name="nama_syarat[]" value="<?php echo ($row['nama_syarat']) ?>" />
                                                        <input type="text" name="kd_syarat_bsw[]" value="<?php echo ($row['kd_syarat_bsw']) ?>" />
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <footer class="blockquote-footer text-danger"><cite title="Source Title">Persyaratan wajib di upload menggunakan format file .pdf dengan max 2MB.</cite></footer>
