    ...
    if (isset($_POST["import"])) {
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                $no_telp_ortu = "";
                if (isset($column[3])) {
                    $no_telp_ortu = mysqli_real_escape_string($conn, $column[3]);
                }
                $ipk = "";
                if (isset($column[4])) {
                    $ipk = mysqli_real_escape_string($conn, $column[4]);
                }
                $gaji_ortu = "";
                if (isset($column[5])) {
                    $gaji_ortu = mysqli_real_escape_string($conn, $column[5]);
                }
                $semester_ke = "";
                if (isset($column[6])) {
                    $semester_ke = mysqli_real_escape_string($conn, $column[6]);
                }
                $pekerjaan_ortu = "";
                if (isset($column[7])) {
                    $pekerjaan_ortu = mysqli_real_escape_string($conn, $column[7]);
                }
                $thn_ajaran = "";
                if (isset($column[8])) {
                    $thn_ajaran = mysqli_real_escape_string($conn, $column[8]);
                }
                $semester = "";
                if (isset($column[9])) {
                    $semester = mysqli_real_escape_string($conn, $column[9]);
                }
                $tahun = "";
                if (isset($column[10])) {
                    $tahun = mysqli_real_escape_string($conn, $column[10]);
                }
                $nominal_pengajuan = "";
                if (isset($column[11])) {
                    $nominal_pengajuan = mysqli_real_escape_string($conn, $column[11]);
                }
                $tgl_daftar = "";
                if (isset($column[12])) {
                    $tgl_daftar = mysqli_real_escape_string($conn, $column[12]);
                }


                $sqlInsert = "INSERT into pendaftaran (kd_daftar, id_mhs, kd_bsw, no_telp_ortu, ipk, gaji_ortu, semester_ke, pekerjaan_ortu, thn_ajaran, semester, tahun, nominal_pengajuan, tgl_daftar)
                       values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
                $paramType = "ssssiiissssss";
                $paramArray = array(
                    $kd_daftar,
                    $id_mhs,
                    $kd_bsw,
