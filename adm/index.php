...
if (strlen($_SESSION['admlogin']) == 0) {
  ...
  tahun
  , kd_bsw
  , SUM(nominal_disetujui) as total_dana_bsw
  FROM pendaftaran
  WHERE kd_bsw = ? and tahun = ? and status = ? GROUP BY kd_bsw";
  $stmt1 = $con->prepare($query1);
  $stmt1->bind_param("sss", $kd_bsw_1, $tahun, $status);
  $stmt1->execute();
  ...
  tahun
  , kd_bsw
  , SUM(nominal_disetujui) as total_dana_pjn
  FROM pendaftaran
  WHERE kd_bsw = ? and tahun = ? and status = ? GROUP BY kd_bsw";
  $stmt2 = $con->prepare($query2);
  $stmt2->bind_param("sss", $kd_bsw_2, $tahun, $status);
  $stmt2->execute();
            ...
            <div class="col-xl-6 col-md-6">
              <div class="card card2 card-stats">
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col">
                      <h5 class="card-title text-uppercase text-muted mb-0">Total dana pinjaman registrasi semester ini</h5>
                      <span class="h2 font-weight-bold mb-0"><?php
                                                              $nominal = $pinjaman_registrasi['total_dana_pjn'];
            ...
            <div class="col-xl-6 col-md-6">
              <div class="card card2 card-stats">
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col">
                      <h5 class="card-title text-uppercase text-muted mb-0">Total dana beasiswa kebutuhan semester ini</h5>
                      <span class="h2 font-weight-bold mb-0"><?php
                                                              $nominal = $beasiswa_kebutuhan['total_dana_bsw'];
                                                              $hasil_rupiah = "Rp " . number_format($nominal, 0, ',', '.');
                                                              echo $hasil_rupiah;  ?></span>
                    </div>
                    <div class="col-auto">
                      <div class="icon icon-shape bg-gradient-primary text-white rounded-circle shadow">
                        <i class="fas fa-hand-holding-usd"></i>
              ...
              var salesChart = new Chart($this, {
                ...
                data: {
                  labels: [<?php
                            ...
                            tahun
                            , tgl_daftar
                            , SUM(nominal_disetujui) as total_dana
                            , semester
                            FROM pendaftaran
                            WHERE kd_bsw= ? AND status= ? GROUP BY tahun";
                            $stmt = $con->prepare($query);
                            $stmt->bind_param("ss", $kd_bsw, $status);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            while ($row = $result->fetch_assoc()) {
                ...
                datasets: [{
                  ...
                  data: [
                    ...
                    $query = "SELECT
                            tahun
                            , tgl_daftar
                            , count(nominal_disetujui) as total_mhs
                            , semester
                            FROM pendaftaran
                            WHERE kd_bsw= ? AND status= ? GROUP BY tahun";
                    $stmt = $con->prepare($query);
